...
pub struct ConfigurePassportOptions {
    ...
    /// Whether to pause the request access program. Cannot be used with
    /// --unpause_request_access.
    #[arg(long)]
    pause_request_access: bool,

    /// Whether to unpause the request access program. Cannot be used with
    /// --pause_request_access.
    #[arg(long)]
    unpause_request_access: bool,
...
pub async fn execute_configure_program(
    ...
    match (pause_request_access, unpause_request_access) {
        ...
        (true, false) => {
            let configure_program_ix = try_build_configure_program_instruction(
                &wallet_key,
                ProgramConfiguration::Flag(ProgramFlagConfiguration::IsRequestAccessPaused(true)),
            )?;
            instructions.push(configure_program_ix);
            compute_unit_limit += 2_000;
        }
        (false, true) => {
            let configure_program_ix = try_build_configure_program_instruction(
                &wallet_key,
                ProgramConfiguration::Flag(ProgramFlagConfiguration::IsRequestAccessPaused(false)),
            )?;
            instructions.push(configure_program_ix);
            compute_unit_limit += 2_000;
        }
