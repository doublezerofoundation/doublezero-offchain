...
- refactor(validator-debt): delete the two dead timestamp to Solana epoch code paths that hardcoded a 0.4s slot duration, along with both `SLOT_TIME_DURATION_SECONDS` constants. Removes two `pub` functions (`ledger::get_solana_epoch_from_dz_epoch`, `rewards::get_rewards_between_timestamps`) and the two private helpers they alone called (`ledger::get_solana_epoch_from_dz_slot`, `rewards::epoch_from_timestamp`), none of which had a caller repo-wide, plus the one test that existed only to exercise them and the two `ValidatorRewards` trait methods (`get_slot`, `get_block_time`) that only the deleted code reached. `JoinedSolanaEpochs::try_new` in `rpc.rs` already performs this mapping for the production path from real block times, and the production worker calls `rewards::get_total_rewards` with an explicit epoch, so no behavior changes. The deletion also removes an unsigned `block_time - dz_block_time` subtraction that panicked in debug builds and wrapped silently in release ones, since the release profile enables no overflow checks. This crate is an internal workspace member and is not published, so the `pub` visibility of the removed items was an artifact of its bin/lib split rather than an API surface. Note that `estimate_block_time_for_skipped_slot` in `rpc.rs` still assumes 0.4s per slot implicitly, as 4 seconds per 10 skipped slots; that path was left alone because the error it contributes is half a second per 10 skipped slots against an epoch-boundary comparison (malbeclabs/infra#2317)
...
- parallelize finalize distribution calls (([#247](https://github.com/doublezerofoundation/doublezero-offchain/pull/247))
