...
- refactor(validator-debt): delete the two dead timestamp to Solana epoch code paths that hardcoded a 0.4s slot duration, along with both `SLOT_TIME_DURATION_SECONDS` constants. Removes four `pub` items with no callers repo-wide: `ledger::get_solana_epoch_from_dz_epoch`, `ledger::get_solana_epoch_from_dz_slot`, `rewards::get_rewards_between_timestamps`, and `rewards::epoch_from_timestamp`. `JoinedSolanaEpochs::try_new` in `rpc.rs` already performs this mapping for the production path using real block times and no slot-duration constant, and the production worker calls `rewards::get_total_rewards` with an explicit epoch, so no behavior changes. The deletion also removes an unsigned subtraction that would panic whenever the DZ block time exceeded the Solana epoch's first block time. This crate is an internal workspace member and is not published, so the `pub` visibility of the removed items was an artifact of its bin/lib split rather than an API surface (malbeclabs/infra#2317)
