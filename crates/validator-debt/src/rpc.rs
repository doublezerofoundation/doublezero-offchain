...
use doublezero_solana_sdk::{
    ...
    revenue_distribution::{
        GENESIS_DZ_EPOCH_MAINNET_BETA, fetch::try_fetch_config, state::Distribution,
...
impl JoinedSolanaEpochs {
    ...
    // (`ingestor::epoch::EpochFinder::find_epoch_at_timestamp`). That one searches
    // forward and backward from an estimated seed slot and does not thread a rate
    // limiter, so the two are not yet worth unifying. A fix to the skipped-slot or
    // boundary handling here probably belongs there too.
...
> {
    ...
    Ok(debt_records.into_iter().zip(distributions).collect())
}
