...
impl WithdrawCommand {
    pub async fn try_into_execute(self, dz_ledger_url: Option<String>) -> Result<()> {
        ...
        let seat_data = accounts[0]
            .as_ref()
            .ok_or_else(|| anyhow::anyhow!("Client seat {client_seat_key} does not exist"))?;
        let (_, _, _, _, active_epoch) = state::parse_client_seat(&seat_data.data)
            .ok_or_else(|| anyhow::anyhow!("Failed to parse client seat {client_seat_key}"))?;
