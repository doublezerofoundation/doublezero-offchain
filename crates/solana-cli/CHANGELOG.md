...
- `shreds pay`: print the fastshreds.com transition notice and require a y/N confirmation before anything is signed or sent. The notice fires before the wallet is built and before the first RPC call, so declining needs neither a loadable keypair nor a reachable cluster. Declining prints `Aborted. Upgrade with ...` and exits 0: a decline is a normal outcome, not a failure, so it does not go through the `Err` path that `main` renders with an `Error:` prefix. When stdin is not a terminal (cron, ansible, piped input) the notice is printed and the command continues rather than prompting, because `read_line` on a closed stdin returns EOF, which the confirmation helper reads as "no" and would abort every automated payer. `--accept-deprecation-notice` skips the prompt on a terminal for batch and multi-seat workflows; the notice still prints (malbeclabs/infra#2164)
- `command`: split the shared interactive prompt into `try_prompt_proceed`, which returns whether the operator chose to proceed, and `try_prompt_proceed_confirmation`, which is now a thin wrapper that bails with the caller's abort message. Existing callers keep their behavior exactly; the new form is for callers that want to report a decline themselves without the `Error:` prefix (malbeclabs/infra#2164)
