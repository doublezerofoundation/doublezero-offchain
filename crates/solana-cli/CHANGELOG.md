...
- `shreds metro-dump`: new read-only command dumping the onchain `MetroHistory` accounts, including the full 32-entry price ring that no existing command could reach (`shreds price` reads only the newest entry). One row per (metro, epoch) with the epoch price, the retransmit-only and current-price-finalized flags, and the initialized device count; `--wide` adds the exchange key, history PDA, bump, and ring `current_index`/`total_count`. Filter with `--metro` (exchange public key or code, e.g. `xlax`), `--epoch` for a single epoch, or `--num-epochs` for the N newest. `Finalized` is populated only on each metro's newest row, since `is_current_price_finalized` describes the current price rather than the history. Metro codes and names come from Serviceability on DZ Ledger and degrade to `?` with a warning when that listing is unreachable, so the Solana-side dump still prints; resolving `--metro` by code is the exception and errors. Backed by a new `parse_metro_history_account` in the SDK, which reads the flags word and walks the ring newest-first, bounded by `total_count` so zero-initialized slots cannot surface as entries at epoch 0 (#406)
