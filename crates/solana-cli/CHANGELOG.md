...
- `shreds`: collapse the two duplicate slot-duration constants (`pay`'s `SLOT_DURATION_SECS` and `publisher-rewards prepare-offchain-message`'s `SLOT_DURATION_MS`) into one `NOMINAL_SLOT_DURATION`, and move it from 400ms to 350ms, matching mainnet-beta from the start of epoch 1020 (2026-08-21). Both users want a reproducible number more than an accurate one, so the value stays cluster-independent: `pay` prints a `~` prefixed epoch-remaining estimate, and `--valid-for` computes a deadline slot that the CLI and the operator have to agree on. Visible effects: `--valid-for 1h` now resolves to 10,285 slots rather than 9,000, and the epoch-remaining estimates shrink by an eighth. Testnet already runs at 200ms, so its estimates stay wrong in the other direction. SIMD-0525 steps mainnet down to 200ms, which will need one more bump here (malbeclabs/infra#2317)
