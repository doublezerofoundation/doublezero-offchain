...
- fix(contributor-rewards): the scheduler no longer writes a snapshot it cannot use. A failed leader-schedule fetch was logged as a warning and thrown away, and the snapshot was uploaded under the canonical `<prefix>-epoch-<N>-snapshot.json` key with a null leader schedule, overwriting any good snapshot for that epoch. The tick then failed reading that file back with "Missing leader schedule", a symptom whose cause existed only in a log line. Both snapshot producers now propagate the fetch error with the epoch in its context, and the scheduler validates before saving, which also closes the `--dry-run` path where nothing validated the snapshot at all. The scheduler logs both failure sites with `{:#}` so the journal carries the cause chain rather than only the outermost context, and `EpochFinder`'s retry logs strip the request URL, which on mainnet-beta carries the read endpoint's API key into journald and Loki (malbeclabs/infra#2372)
