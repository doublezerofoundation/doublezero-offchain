...
## [Unreleased]
- remove the testnet shred-subscription DZ Ledger special-case: drop `NetworkEnvironment::shred_subscription_url()` and `SolanaConnectionOptions::into_shred_subscription_connection()`, replacing the latter with the generic `into_solana_connection()` that resolves `-u`/`--url` to the network's Solana RPC URL for every moniker. The testnet shred-subscription program now lives on Solana devnet ([infra #1763](https://github.com/malbeclabs/infra/issues/1763))
